...
def train_val_test_split(df: pd.DataFrame, target_column: str = 'Class') -> Tuple[
    ...
    print("-----------------------------")
    print("X_train shape:", X_train.shape, "Pct:", f"{round(X_train.shape[0]/X.shape[0]*100,2)}%")
    print("y_train shape:", y_train.shape)
    print("-----------------------------")
    print("X_val shape:", X_val.shape, "Pct:", f"{round(X_val.shape[0]/X.shape[0]*100,2)}%")
    print("y_val shape:", y_val.shape)
    print("-----------------------------")
    print("X_test shape:", X_test.shape, "Pct:", f"{round(X_test.shape[0]/X.shape[0]*100,2)}%")
...
class ApplySmote:
    ...
    @staticmethod
    def fit_resample(X_train: pd.DataFrame, y_train: pd.Series, 
                     random_state=42, k_neighbors=5) -> Tuple[pd.DataFrame, pd.Series]:
        """
    ...
    def _plot_class_distribution(y_before: pd.Series, y_after: pd.Series) -> None:
        ...
        for i, (class_label, count) in enumerate(counts_before.items()):
            ...
            axes[0].annotate(f'{count:,} ({percentage:.1f}%)', 
                           (i, count), 
                           ha='center', va='bottom', 
        ...
        for i, (class_label, count) in enumerate(counts_after.items()):
            ...
            axes[1].annotate(f'{count:} ({percentage:.1f}%)', 
                           (i, count), 
                           ha='center', va='bottom', 
                           fontweight='bold', fontsize=10)
